...
open class LandscapeBatteryDrawableOneUI7(private val context: Context, frameColor: Int) : Drawable() {
    ...
    private var isAnimatingOut: Boolean = false // Flag to determine direction of animation
        ...
        set(value) {
            if (field != value) {
                ...
                if (stateChange != previousState) {
                    // Store current state before updating for animation direction check
    ...
    private fun startSlideAnimation() {
        ...
        // We're animating out if going back to normal state from charging or power save
        isAnimatingOut = previousState != STATE_NORMAL && 
                         (previousState == STATE_CHARGING && !charging) || 
                         (previousState == STATE_POWER_SAVE && !powerSaveEnabled)
    ...
    override fun draw(c: Canvas) {
        ...
        // Calculate animation progress for slide in/out effect
        ...
        if (slideAnimationActive) {
            ...
            if (elapsedTime < SLIDE_ANIMATION_DURATION_MS) {
                ...
                // If animating out, invert the progress (1.0 -> 0.0 instead of 0.0 -> 1.0)
                slideProgress = if (isAnimatingOut) 1.0f - progress else progress
                
                // Schedule next frame
                invalidateSelf()
            } else {
                slideAnimationActive = false
                slideProgress = if (isAnimatingOut) 0.0f else 1.0f
            }
        }
        ...
        if ((charging || powerSaveEnabled) && batteryLevel < 100) {
            ...
            // Apply animation - text slides in from the edge or out to the edge
            val startX = bounds.width() * 1.5f // Off-screen position
            textXOffset = if (isAnimatingOut && previousState == STATE_NORMAL) {
                // Slide out to the right when returning to normal
                baseTextX + (startX - baseTextX) * (1.0f - slideProgress)
            } else {
                // Standard slide in from right
                startX - (startX - baseTextX) * slideProgress
            }
        } else {
        ...
        // Handle icon animations (both forward and reverse)
        val leftEdgeX = -bounds.width() * 0.5f // Off-screen to the left
            
        if (charging || (isAnimatingOut && previousState == STATE_CHARGING)) {
            // Show charging icon with animation
            var iconProgress = slideProgress
            
            // Calculate animation for bolt icon
            if (isAnimatingOut && previousState == STATE_CHARGING) {
                // Animate bolt icon out to the left
                iconXOffset = baseIconOffsetX + (leftEdgeX - baseIconOffsetX) * (1.0f - slideProgress)
                
                // Only show if we're still animating
                if (slideProgress > 0) {
        ...
        } else if (powerSaveEnabled || (isAnimatingOut && previousState == STATE_POWER_SAVE)) {
            // Show battery saver icon with animation
            if (isAnimatingOut && previousState == STATE_POWER_SAVE) {
                // Animate battery saver icon out to the left
                iconXOffset = baseIconOffsetX + (leftEdgeX - baseIconOffsetX) * (1.0f - slideProgress)
